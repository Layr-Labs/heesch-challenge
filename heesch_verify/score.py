...
def _defect_fraction(defect: int, required: int, present: bool) -> Fraction:
    if not present or required <= 0:
        return _WORST
...
def hc_board_key(r: Result) -> tuple:
    ...
    frac = _defect_fraction(r.defect_hc, r.defect_required, r.defect_block_present)
    return (
        r.hc_verified,
        -frac,
        r.hh_verified,
...
def hh_board_key(r: Result) -> tuple:
    ...
    frac = _defect_fraction(r.defect_hh, r.defect_required, r.defect_block_present)
    return (
        r.hh_verified,
        -frac,
        r.hc_verified,
...
def yukon_score(r: Result) -> float:
    ...
    never be labelled as one (§9.2.6)."""
    base = float(r.hc_verified)
    if not r.defect_block_present or r.defect_required <= 0:
        return base
